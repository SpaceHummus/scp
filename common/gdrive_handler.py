...
class GDriveHandler:
    ...
    # get the bundle_adjustment file for G-Drive
    def get_bundle_adjustment_file(self, cameras, focus_level):
        logging.info("Getting configuration.yaml from G-Drive...")
        folder_id = self.main_folder_id
        file_id = ""
        file_list = self.drive.ListFile({'q': "'%s' in parents and trashed=false" % folder_id}).GetList()
        for f in file_list:
            ...
            if title == COMMANDS_FOLDER:
                ...
                for i in file_list2:
                    logging.debug('title: %s, id: %s' % (i['title'], i['id']))
                    title = i['title']
                    if title == f"bundle_adjustment_{cameras}_F{focus_level}.yaml":
                        file_id = i['id']
        if file_id == "":
            logging.error(f"unable to find bundle_adjustment_{cameras}_F{focus_level}.yaml on G-Drive")
            return None
        else:
            file = self.drive.CreateFile({'id': file_id})
            bundle_data = file.GetContentString(f"bundle_adjustment_{cameras}_F{focus_level}.yaml")
            return bundle_data
        # download images by start date, end date, list cameras, list focuses, path
    ...
    def get_image_id(self, the_date, the_camera, the_focus):
        ...
        file_list = self.drive.ListFile(
            {'q': "'%s' in parents and trashed=false and title = '%s'" % (folder_id, file_name)}).GetList()
        for f in file_list:
            try:
                logging.debug('title: %s, id: %s' % (f['title'], f['id']))
                return f['id']
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <= 1:
        ...
        print(
            "supported commands: new_exp - create a new experiment with new folder structures. parametrs: <experiment name> <experiment date in YYYY/M/D hh:mm:ss> . prints the new folder id of the experiment\n")
        print(
            "for example: python3 gdrive_handler.py new_exp \"2021-07-08 B0.1 Yerucham Dev1\" \"2021/07/15 10:00:00\"")
        quit()
    else:
        cmd = sys.argv[1]
        if cmd == "new_exp":
